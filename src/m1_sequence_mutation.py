...
def run_test_zero_changer():
    ...
    # Test 3:
    test3 = ([5,6,9,4,7,0,1,0], [0])
    expected3 = ([5,6,9,4,7,1,1,2], [3])
    zero_changer(test3)
    print()
    print('Test 3:')
...
def zero_changer(tuple_of_lists):
    ...
    for k in range(len(tuple_of_lists)):
        for l in range(len(tuple_of_lists[k])):
            if (tuple_of_lists[k])[l] == 0:
                (tuple_of_lists[k])[l] = zeros
                zeros = zeros + 1
